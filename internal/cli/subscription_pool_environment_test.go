...
func TestProviderOnlyEnvironmentRemovesInheritedClaudeAuth(t *testing.T) {
	t.Parallel()
...
func TestExplicitClaudeAccountRequiresSubscriptionPool(t *testing.T) {
	...
	err = validateLaunchInputs("", launchAuthModeAuto, "personal", "")
	if err == nil || !strings.Contains(err.Error(), "--auth-mode subscription-pool") {
		t.Fatalf("validateLaunchInputs auto error = %v", err)
	}
