...
func TestNativeAnthropicStreamPreservesExplicitAgentModel(t *testing.T) {
	...
	if err := adapter.Start(writer); err != nil {
		t.Fatalf("adapter.Start() error = %v", err)
	}
	events := []upstreamStreamEvent{
		{kind: upstreamStreamData, sseEvent: "content_block_start", data: []byte(`{"type":"content_block_start","index":0,"content_block":{"type":"tool_use","name":"Agent","input":{}}}`)},
		{kind: upstreamStreamData, sseEvent: "content_block_delta", data: []byte(`{"type":"content_block_delta","index":0,"delta":{"type":"input_json_delta","partial_json":"{\"prompt\":\"work\",\"description\":\"work\",\"model\":\"haiku\"}"}}`)},
		{kind: upstreamStreamData, sseEvent: "content_block_stop", data: []byte(`{"type":"content_block_stop","index":0}`)},
		{kind: upstreamStreamData, sseEvent: "message_stop", data: []byte(`{"type":"message_stop"}`)},
	}
	for _, event := range events {
		if err := adapter.Consume(writer, event); err != nil {
			t.Fatalf("adapter.Consume(%s) error = %v", event.sseEvent, err)
		}
	}
	writer.terminal = true
	body := recorder.Body.String()
	if !strings.Contains(body, `\"model\":\"haiku\"`) || strings.Contains(body, "anthropic.ccr.worker") {
		t.Fatalf("Agent stream changed native model = %s", body)
	}
}

func TestNativeAnthropicStreamNormalizesAgentInputBeforeExposure(t *testing.T) {
	adapter := &nativeAnthropicStreamAdapter{}
	recorder := httptest.NewRecorder()
	writer := newAnthropicSSEWriter(recorder)
	if err := writer.Start(); err != nil {
	...
	for _, event := range []upstreamStreamEvent{
		{kind: upstreamStreamData, sseEvent: "content_block_start", data: []byte(`{"type":"content_block_start","index":0,"content_block":{"type":"tool_use","name":"Agent","input":{}}}`)},
		{kind: upstreamStreamData, sseEvent: "content_block_delta", data: []byte(`{"type":"content_block_delta","index":0,"delta":{"type":"input_json_delta","partial_json":"{\"prompt\":\"research routing\",\"agent_type\":\"general-purpose\",\"provider_only\":\"drop-me\"}"}}`)},
		{kind: upstreamStreamData, sseEvent: "content_block_stop", data: []byte(`{"type":"content_block_stop","index":0}`)},
		{kind: upstreamStreamData, sseEvent: "message_stop", data: []byte(`{"type":"message_stop"}`)},
	} {
		if err := adapter.Consume(writer, event); err != nil {
	...
	body := recorder.Body.String()
	for _, want := range []string{`\"description\":\"research routing\"`, `\"subagent_type\":\"general-purpose\"`} {
		if !strings.Contains(body, want) {
			t.Fatalf("normalized stream missing %s: %s", want, body)
		}
	}
	for _, forbidden := range []string{`\"agent_type\"`, `\"provider_only\"`} {
		if strings.Contains(body, forbidden) {
			t.Fatalf("normalized stream retained %s: %s", forbidden, body)
		}
	}
}

func TestNativeAnthropicStreamRegistersAgentBeforeMessageStop(t *testing.T) {
	var registered int
	adapter := &nativeAnthropicStreamAdapter{
		agentChildMarker: func(descriptors []agentChildDescriptor) (func(), error) {
			registered = len(descriptors)
			return func() { registered = 0 }, nil
		},
	}
	recorder := httptest.NewRecorder()
	writer := newAnthropicSSEWriter(recorder)
	...
	for _, event := range []upstreamStreamEvent{
		{kind: upstreamStreamData, sseEvent: "content_block_start", data: []byte(`{"type":"content_block_start","index":0,"content_block":{"type":"tool_use","name":"Agent","input":{}}}`)},
		{kind: upstreamStreamData, sseEvent: "content_block_delta", data: []byte(`{"type":"content_block_delta","index":0,"delta":{"type":"input_json_delta","partial_json":"{\"prompt\":\"work\"}"}}`)},
		{kind: upstreamStreamData, sseEvent: "message_stop", data: []byte(`{"type":"message_stop"}`)},
	} {
		if err := adapter.Consume(writer, event); err != nil {
			t.Fatal(err)
	...
	if registered != 1 {
		t.Fatalf("registered Agent children = %d, want 1 before message_stop was exposed", registered)
	}
}

func TestNativeAnthropicStreamKeepsExposedAgentReservationAfterTrailingError(t *testing.T) {
	rolledBack := false
	adapter := &nativeAnthropicStreamAdapter{
	...
	if !adapter.childToolsExposed {
		t.Fatal("child tool was not marked exposed after message_stop")
	}
	if err := adapter.Consume(writer, upstreamStreamEvent{kind: upstreamStreamData, sseEvent: "error", data: []byte(`{"type":"error","error":{"type":"api_error"}}`)}); err != nil {
		t.Fatalf("trailing error Consume() error = %v", err)
	}
	// The stream coordinator invokes Cleanup directly when the upstream emits a
	// failure, before it has a chance to call Finish.
	adapter.Cleanup()
	if rolledBack {
		t.Fatal("stream cleanup rolled back an already exposed child reservation")
	}
	if _, err := adapter.Finish(writer); err == nil {
		t.Fatal("Finish() succeeded after trailing provider error")
	}
	if rolledBack {
		t.Fatal("trailing provider error rolled back an already exposed child reservation")
	}
}

func TestTranslatedProviderStreamReportsExposedChildAfterTrailingError(t *testing.T) {
	...
	result := runTranslatedProviderStreamWithTiming(
		...
		func(ctx context.Context, events chan<- upstreamStreamEvent) {
			...
			for _, event := range []upstreamStreamEvent{
				...
				if !sendUpstreamStreamEvent(ctx, events, event) {
					return
				}
			}
			sendUpstreamStreamEvent(ctx, events, upstreamStreamEvent{
				kind:    upstreamStreamFailed,
				failure: &streamFailure{errorClass: "provider_stream", message: "trailing provider failure"},
			})
		},
		adapter,
		time.Second,
	...
	if !result.Committed || !result.ChildToolsExposed || result.TerminalPhase != "failed" {
		t.Fatalf("stream result = %#v, want committed failed result with exposed child", result)
	}
	if rolledBack {
		t.Fatal("coordinator cleanup rolled back an already exposed child reservation")
	}
}

func TestNativeAnthropicStreamSurfacesMalformedChildAsCompatibilityText(t *testing.T) {
	adapter := &nativeAnthropicStreamAdapter{responseModel: "provider-model"}
	recorder := httptest.NewRecorder()
	writer := newAnthropicSSEWriter(recorder)
	if err := writer.Start(); err != nil {
		t.Fatal(err)
	...
	for _, event := range []upstreamStreamEvent{
		{kind: upstreamStreamData, sseEvent: "content_block_start", data: []byte(`{"type":"content_block_start","index":0,"content_block":{"type":"tool_use","name":"Agent","input":{}}}`)},
		{kind: upstreamStreamData, sseEvent: "content_block_delta", data: []byte(`{"type":"content_block_delta","index":0,"delta":{"type":"input_json_delta","partial_json":"{\"model\":\"haiku\"}"}}`)},
	} {
		if err := adapter.Consume(writer, event); err != nil {
			t.Fatalf("adapter.Consume(%s) error = %v", event.sseEvent, err)
		}
	}
	err := adapter.Consume(writer, upstreamStreamEvent{kind: upstreamStreamData, sseEvent: "message_stop", data: []byte(`{"type":"message_stop"}`)})
	if err != nil {
		t.Fatalf("malformed child compatibility response error = %v", err)
	}
	body := recorder.Body.String()
	if strings.Contains(body, `"type":"tool_use"`) || strings.Contains(body, `"name":"Agent"`) {
		t.Fatalf("malformed child tool was exposed: %s", body)
	}
	if !strings.Contains(body, "invalid Agent tool input") ||
		!strings.Contains(body, `"stop_reason":"end_turn"`) ||
		!strings.Contains(body, "event: message_stop") {
		t.Fatalf("missing visible malformed-child compatibility response: %s", body)
	}
}

func TestNativeAnthropicStreamSuppressesPrecedingToolWhenChildIsMalformed(t *testing.T) {
	adapter := &nativeAnthropicStreamAdapter{responseModel: "provider-model"}
	recorder := httptest.NewRecorder()
	writer := newAnthropicSSEWriter(recorder)
	if err := writer.Start(); err != nil {
	...
	for _, event := range []upstreamStreamEvent{
		{kind: upstreamStreamData, sseEvent: "content_block_start", data: []byte(`{"type":"content_block_start","index":0,"content_block":{"type":"tool_use","name":"Bash","id":"toolu_bash","input":{}}}`)},
		{kind: upstreamStreamData, sseEvent: "content_block_delta", data: []byte(`{"type":"content_block_delta","index":0,"delta":{"type":"input_json_delta","partial_json":"{\"command\":\"inspect\"}"}}`)},
		{kind: upstreamStreamData, sseEvent: "content_block_stop", data: []byte(`{"type":"content_block_stop","index":0}`)},
		{kind: upstreamStreamData, sseEvent: "content_block_start", data: []byte(`{"type":"content_block_start","index":1,"content_block":{"type":"tool_use","name":"Task","input":{}}}`)},
		{kind: upstreamStreamData, sseEvent: "content_block_delta", data: []byte(`{"type":"content_block_delta","index":1,"delta":{"type":"input_json_delta","partial_json":"{\"model\":\"haiku\"}"}}`)},
		{kind: upstreamStreamData, sseEvent: "message_stop", data: []byte(`{"type":"message_stop"}`)},
	} {
		if err := adapter.Consume(writer, event); err != nil {
			t.Fatalf("adapter.Consume(%s) error = %v", event.sseEvent, err)
	...
	body := recorder.Body.String()
	if strings.Contains(body, `"name":"Bash"`) || strings.Contains(body, `"toolu_bash"`) {
		t.Fatalf("preceding tool call was exposed with malformed child: %s", body)
	}
	if !strings.Contains(body, "invalid Task tool input") || !strings.Contains(body, "event: message_stop") {
		t.Fatalf("missing malformed-child compatibility response: %s", body)
	}
	if !strings.Contains(body, `"index":0`) {
		t.Fatalf("malformed-child replacement block was not contiguous with visible output: %s", body)
	}
}
