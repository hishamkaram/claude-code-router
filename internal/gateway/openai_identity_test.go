...
func TestGatewaySuppressesRouteIdentityWhenSystemMessagesUnsupported(t *testing.T) {
	ctx := context.Background()
	var gotMessages []openAIMessage
	provider := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		var payload struct {
		...
		gotMessages = payload.Messages
		w.Header().Set("Content-Type", "application/json")
		_, _ = fmt.Fprint(w, `{"id":"chatcmpl-test","choices":[{"message":{"content":"unknown"},"finish_reason":"stop"}]}`)
	}))
	defer provider.Close()

	s := newGatewayStore(t, store.Provider{Name: "litellm", Type: "litellm", BaseURL: provider.URL}, store.Model{
		Alias: "glm", ProviderName: "litellm", ProviderModel: "glm-5.2", Status: "degraded",
		CapabilityOverrides: modelcap.Values{SupportsSystemMessages: modelcap.Bool(false)},
	})
	server := startGateway(t, ctx, s, fakeGatewaySecrets{})
	defer func() { _ = server.Shutdown(ctx) }()

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, server.URL()+"/v1/messages", strings.NewReader(`{"model":"anthropic.ccr.glm","messages":[{"role":"user","content":"which model are you now?"}]}`))
	if err != nil {
		t.Fatalf("NewRequest() error = %v", err)
	}
	req.Header.Set("Authorization", "Bearer local-token")
	...
	if len(gotMessages) != 1 || gotMessages[0].Role != "user" {
		t.Fatalf("provider messages = %#v, want only the user message", gotMessages)
	}
}
